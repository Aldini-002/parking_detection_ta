...
def manual_book():
    ...
    header = ctk.CTkLabel(
        frame_header, text="Manual Book", font=("Helvetica", 15, "bold")
    )
    ...
    tree = ttk.Treeview(
        frame_body, columns=("#", "Key", "Function"), show="headings", height=0
    )
    tree.heading("#", text="#")
    ...
    datas = [
        {"key": "i", "function": "Show manual book"},
        {"key": "left click", "function": "Draw the polyline"},
        {"key": "right click", "function": "Delete the polyline"},
        {"key": "s", "function": "Save"},
        {"key": "q", "function": "Quit"},
    ]
...
def setup_parkings(name, old_name, source, location, case):
    ...
    model = YOLO(os.path.join(paths["model_path"], "toy.pt"))
